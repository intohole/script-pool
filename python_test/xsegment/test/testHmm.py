...
'm': {'s': 1.,  'b': 1., 'm': 1., 'e': 1.}, 'e': {'s': 1.,  'b': 1., 'm': 1., 'e': 1.}}
    ...
    def add_line(self, line):
        # if not ( line  and not isinstance(line , basestring) ) :
...
if __name__ == '__main__':
    # h = HSegment()
    # print ' '.join(h.segment(u'南京市长江大桥今天竣工！'))
    # print ' '.join(h.segment(u'理想很远大，现实很骨干'))
    # print ' '.join(h.segment(u'做我女朋友好不好?'))
    ...
    for file_name in os.listdir(chinese):
        ...
        for line in content[6:]:
            line = line.strip().replace(' [%s]' % wd , wd).replace(' [%s] ' % wd , wd).replace('[%s] '  % wd , wd).split("\t")[2]
            if line in diff:
                continue
            diff.add(line)
            t.add_line(' '.join(word.split(line)).strip())
    
    with open('d:/english.txt') as f:
