...
def bind_api(**config):
    ...
    class APIMethod(object):
        ...
        def execute(self):
            ...
            self.parameters.setdefault('access_token',self.api.auth.access_token)
            ...
            if resp.status != 200:
                try:
                    ...
                    error =  json['error']
                    error_msg = "%s,%s" % (error_code,error)#'error_code:' + error_code +','+ error
                    print error_msg
                except Exception:
                    error_msg = "Twitter error response: status code = %s,error_code=%d" % (resp.status,error_code)
