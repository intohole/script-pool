...
class API(object):
    ...
    def __init__(self, auth_handler=None,
            host='api.weibo.com', search_host='api.weibo.com',
             cache=None, secure=True, api_root='/2', search_root='',
            retry_count=0, retry_delay=0, retry_errors=None,source=None,
            parser=None, log = None):
    ...
    get_havetimes = bind_api(
        path = '/account/rate_limit_status.json',
        payload_type ='followers' , payload_list = False,
        allowed_param = [],
        require_auth = True
    )
    """ statuses/comment """
    ...
    """ 
      sina2.0
      path : sinaweibo app internet
      payload_type : explain return,find it in the model 
      payload_list = the return 
      allowed_param : the sinaweibo allowed the input 
           
    """
    """ followers id"""
    l_followers_ids = bind_api(
        path = '/friendships/followers/ids.json',
        payload_type = 'followers', payload_list = False,
        allowed_param = [ 'user_id', 'screen_name', 'count', 'cursor']
    )
    '''active followers'''
    l_followers_active_ids = bind_api(
        path = 'friendships/followers/active.json',
        payload_type = 'user', payload_list = True,
        allowed_param = ['uid','count'],
        require_auth = True      
    )
    ''' user information'''
    l_users_show_uid = bind_api(
        path = '/users/show.json',
    ...
    """
       sina suggestion user
       category: job type 
    """
    l_suggest_user_hot = bind_api(
        path = '/suggestions/users/hot.json',
        payload_type = 'user', payload_list = True,
        allowed_param = ['category'],
        require_auth = True
    )
    """
        input : you can input one more uid, but you need divide with the point
        output: list
        function : get uid which you input ,follower count status count,friend cout
    """
    l_user_counts = bind_api(
        path = '/users/counts.json',
        payload_type = 'json', payload_list = True,
        allowed_param = ['uids']
    )
    l_user_tag = bind_api(
        path = '/tags/tags_batch.json',
