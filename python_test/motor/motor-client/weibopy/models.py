    ...
    def parse(cls, api ,json):
        ...
        for k,v in json.items():
            if k == 'id':
                ...
                tag = []
                for user_tags in v:
                    for tag_id,tag_v in user_tags.items():
                        if tag_id != 'weight':
                            tag.append(tag_v)
                setattr(ids , k , tag)
        return ids      
class ModelFactory(object):
    ...
    """
    """
    this is factory model. you need add the interface name in the down file 
    you can create class in the up 
    you can look the other class  
    exp : ids = IDSModel(Model) 
    the mead is ids is model instance of the class idsmodel 
    fuck the man who write this program    
    """
    status = Status
    comments = Comments
    user = User
